...
| Extraction refinements | ✅ Done | **Even-spaced sampling** (ffprobe duration → `fps = maxFrames/duration`), **perceptual dedup** (9×8 dHash, Hamming < 6), **blur scoring** (64×64 Laplacian variance → drops motion-blurred frames, persisted on `ExtractionFrame.blurScore`) — all from a single 3-output ffmpeg pass. **Product enrich** — analyzer dedups vision products by normalized title (keeps most confident) + orders best-first for the review grid. Verified by real-ffmpeg tests (even-spacing, dedup-collapse, sharp > blurred) + pure units; full gates green. |
| CLIP semantic dedup | ✅ Done (gated) | `@aicos/ai-core` `ImageEmbedder` (+ `cosineSimilarity` / `semanticDedupeIndices`) + a gated `HttpImageEmbedder` (CLIP inference server behind `CLIP_EMBED_URL`). Worker `SemanticDeduperService` embeds the sampled frames and drops semantic near-dupes (cosine ≥ `CLIP_DEDUP_THRESHOLD`) above the pixel dHash layer — **graceful no-op** when unconfigured / on error / on count mismatch. Tested with mock fetch (pure math + embedder + service); worker 45 tests, full gates green. Runbook `live-integrations.md` §5. |

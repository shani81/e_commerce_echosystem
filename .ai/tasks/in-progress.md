...
| Live AI remaining | 🟦 Next | End-to-end live run (upload→sample→Gemini→review) needs **MinIO/S3 browser CORS** + a `GEMINI_API_KEY`. Then refine: even-spaced sampling (ffprobe duration), blur/`phash` frame dedup, CLIP/enrich. Also: capture `order.shippingAddress` for Shippo (held). |
